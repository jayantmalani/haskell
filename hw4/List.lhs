...
\begin{code}
{-@ length:: xS:List a -> {n:Nat| n == size xS} @-}
length            :: List a -> Int
length Emp        = 0
...
{-@ measure concats :: List (List a) -> Int
    concats Emp          = 0
    concats ((:+:) x xs) = size x + concats xs
@-}

{-@ concat :: xss:List (List a) -> ListN a (concats xss)  @-}
concat Emp = Emp
concat (x:+:xss) = x `append` concat xss
