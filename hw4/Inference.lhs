...
ti env (EFst e)       =
  do
    ...
    tvL <- freshTVbl "a"
    tvR <- freshTVbl "b"
    s2  <- mgu (TCom tvL tvR) t1
    let s = s2 `after` s1
    return (s, apply s tvL)
...
ti env (EIsNil e)      =
  do (s1,t1) <- infer env e
     return (s1, TBool)
     
ti env (EDcons e)      =
  do
    (s1,t1)  <- infer env e
    return   (s1 , t1 `TCom` TList t1)

ti env (ERec x e1 e2) = do  
                            tx <- freshTVbl "a"
                            let 
                                env1 = env \\ (x, Forall [] tx)
                            (s1, t1) <- ti env1 e1
                            s2 <- mgu (apply s1 tx) t1
                            let 
                                s3  = s2 `after` s1
                                env2 = apply s3 env1
                                t    = generalize env2 (apply s3 t1)
                                env3 = env2 \\ (x, t)
                            (s4, t2) <- ti env3 e2
...
test e = case typeInference (TypeEnv env) e of
...
prExp (EDcons e)       = PP.text "DCons " PP.<+> prParenExp e
prExp _                = PP.text "unknown"
