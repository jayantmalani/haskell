...
{-@ measure add  @-}
add :: (Variable,Value) -> S.Set Variable
add (x,y) = S.singleton x

{-@ measure vars @-}
vars :: Store -> S.Set Variable
vars [] = S.empty
vars (x:xs) = add x `S.union` vars xs

{-@ inline has @-}
has :: Variable -> Store -> Bool
has k m = S.member k s
        where
          s = vars m
...
\begin{code}
{-@ type ScopeState G = {s:Statement | wellScoped s G}  @-}

{-@ inline wellScoped @-}
wellScoped :: Statement -> Store -> Bool
wellScoped st s =  S.isSubsetOf (readS st) (vars s)

{-@ evalS :: Store -> st:Statement -> {s:Store | wellScoped st s} @-}
...
\begin{code}
{-@ inline isSub @-}
isSub :: (Ord a) => S.Set a -> S.Set a -> Bool
isSub s1 s2 = S.isSubsetOf s1 s2

{-@ measure free @-}
free :: Expression -> S.Set Variable
free (Var x) = S.singleton x
free (Val _) = S.empty
free (Op _ e1 e2) = S.union t1 t2
                  where
                    t1 = free e1
                    t2 = free e2

{-@ evalE :: s:Store -> { e:Expression | isSub (free e) (vars s)}-> Value @-}
...
readS (Assign x e)     = readE e    -- TODO: replace with proper definition 
readS (IfZ e s1 s2)    = S.union (readE e) (S.union (readS s1) (readS s2))    -- TODO: replace with proper definition
readS (WhileZ e s)     = S.union (readE e) (readS s)    -- TODO: replace with proper definition
readS (Sequence s1 s2) = S.union x (S.difference y z)
                         where
                           x = readS s1
                           y = readS s2
                           z = written s1 -- TODO: replace with proper definition
readS Skip             = S.empty    -- TODO: replace with proper definition

{-@ measure written @-}
written :: Statement -> S.Set Variable
written (Assign x e)  = S.singleton x
written (IfZ e s1 s2) = S.intersection (written s1) (written s2)
written (WhileZ e s)  = if (checkk e)
                        then written s
                        else S.empty
written (Sequence s1 s2) = S.union (written s1) (written s2)
written Skip          = S.empty

{-@ measure checkk @-}
checkk :: Expression -> Bool
checkk (Val v) = if v > 0
                 then True
                 else False
checkk _       = True
...
readE (Var x)          = S.singleton x    -- TODO: replace with proper definition
readE (Val v)          = S.empty    -- TODO: replace with proper definition
readE (Op o e1 e2)     = S.union t1 t2
                       where
                         t1 = (readE e1)
                         t2 = (readE e2) -- TODO: replace with proper definition
\end{code}
